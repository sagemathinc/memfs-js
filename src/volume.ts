...
function optsAndCbGenerator<TOpts, TResult>(
  getOpts
): (options, callback?) => [TOpts, TCallback<TResult>] {
...
const getReadFileOptions =
  optsGenerator<IReadFileOptions>(readFileOptsDefaults);
...
const getAppendFileOptsAndCb = optsAndCbGenerator<IAppendFileOptions, void>(
  getAppendFileOpts
);
...
const getRealpathOptsAndCb = optsAndCbGenerator<IRealpathOptions, TDataOut>(
  getRealpathOptions
);
...
const getStatOptions: (options?: any) => IStatOptions = (options = {}) =>
  Object.assign({}, statDefaults, options);
const getStatOptsAndCb: (
  options: any,
  callback?: TCallback<Stats>
) => [IStatOptions, TCallback<Stats>] = (options, callback?) =>
  typeof options === "function"
...
let resolve: TResolve = (filename, base = process.cwd()) =>
...
export class Volume {
  ...
  */
  ...
  private openFile(
    ...
    const steps = filenameToSteps(filename);
    let link: Link | null = resolveSymlinks
    ...
    if (!link && flagsNum & O_CREAT) {
      ...
      const dirLink: Link | null = this.getResolvedLink(
  ...
  readFile(
    id: TFileId,
    a: TCallback<TDataOut> | IReadFileOptions | string,
    b?: TCallback<TDataOut>
  ) {
    const [opts, callback] = optsAndCbGenerator<
      IReadFileOptions,
      TCallback<TDataOut>
    >(getReadFileOptions)(a, b);
    const flagsNum = flagsToNumber(opts.flag);
  ...
  writeFile(
    id: TFileId,
    data: TData,
    a: TCallback<void> | IWriteFileOptions | string,
    b?: TCallback<void>
  ) {
  ...
  symlink(
    target: PathLike,
    path: PathLike,
    a: symlink.Type | TCallback<void>,
    b?: TCallback<void>
  ) {
    const callback: TCallback<void> = validateCallback(
  ...
  private realpathBase(
    filename: string,
    encoding: TEncodingExtended | undefined
  ): TDataOut {
    const steps = filenameToSteps(filename);
  ...
  realpathSync(path: PathLike, options?: IRealpathOptions | string): TDataOut {
    return this.realpathBase(
      pathToFilename(path),
      getRealpathOptions(options).encoding
    );
  }
  ...
  realpath(
    path: PathLike,
    a: TCallback<TDataOut> | IRealpathOptions | string,
    b?: TCallback<TDataOut>
  ) {
  ...
  lstat(
    path: PathLike,
    a: TCallback<Stats> | IStatOptions,
    b?: TCallback<Stats>
  ): void {
    const [{ throwIfNoEntry = true, bigint = false }, callback] =
      getStatOptsAndCb(a, b);
    this.wrapAsync(
      this.lstatBase,
      [pathToFilename(path), bigint, throwIfNoEntry],
      callback
    );
  }
  ...
  stat(
    ...
    const [{ bigint = false, throwIfNoEntry = true }, callback] =
      getStatOptsAndCb(a, b);
  ...
  fstat(
    fd: number,
    a: TCallback<Stats> | IFStatOptions,
    b?: TCallback<Stats>
  ): void {
  ...
  appendFileSync(
    id: TFileId,
    data: TData,
    options: IAppendFileOptions | string = appendFileDefaults
  ) {
  ...
  utimes(
    ...
    this.wrapAsync(
      this.utimesBase,
      [pathToFilename(path), toUnixTimestamp(atime), toUnixTimestamp(mtime)],
      callback
    );
  }
  ...
  mkdir(
    path: PathLike,
    a: TCallback<void> | TMode | IMkdirOptions,
    b?: TCallback<void>
  ) {
  ...
  rmdir(
    path: PathLike,
    a: TCallback<void> | IRmdirOptions,
    b?: TCallback<void>
  ) {
  ...
  watch(
    ...
    let { persistent, recursive, encoding }: IWatchOptions =
