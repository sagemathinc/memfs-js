...
describe("volume", () => {
  describe("filenameToSteps(filename): string[]", () => {
    ...
    it('/usr/bin/node.sh -> ["usr", "bin", "node.sh"]', () => {
      expect(filenameToSteps("/usr/bin/node.sh")).toEqual([
        "usr",
        "bin",
        "node.sh",
      ]);
  ...
  describe("Volume", () => {
    ...
    describe(".fromJSON(json[, cwd])", () => {
      ...
      it("Invalid JSON throws error", () => {
        try {
          ...
          expect(error.code === "EISDIR" || error.code === "ENOTDIR").toBe(
            true
          );
        }
      });
    ...
    describe(".symlinkSync(target, path[, type])", () => {
      ...
      describe("Complex, deep, multi-step symlinks get resolved", () => {
        it("Symlink to a folder", () => {
          const vol = Volume.fromJSON({
            "/a1/a2/a3/a4/a5/hello.txt": "world!",
          });
          vol.symlinkSync("/a1", "/b1");
          expect(vol.readFileSync("/b1/a2/a3/a4/a5/hello.txt", "utf8")).toBe(
            "world!"
          );
        });
        ...
        it("Multiple hops to folders", () => {
          ...
          expect(
            vol.readFileSync("/c1/c2/c3/c4/c5/final/a3/a4/a5/hello.txt", "utf8")
          ).toBe("world a");
        });
      });
    });
    ...
    describe(".statSync(path, options)", () => {
      ...
      it("Throws when entry does not exist if throwIfNoEntry is explicitly undefined", () => {
        expect(() =>
          vol.statSync("/foo", { throwIfNoEntry: undefined })
        ).toThrow();
      });
    });
    describe(".lstatSync(path, options)", () => {
      ...
      it("Throws when entry does not exist if throwIfNoEntry is explicitly undefined", () => {
        expect(() =>
          vol.lstatSync("/foo", { throwIfNoEntry: undefined })
        ).toThrow();
      });
    });
    describe(".lstatSync(path)", () => {
      ...
      it("Returns file stats using BigInt", () => {
        if (hasBigInt) {
          ...
          expect(() =>
            vol.lstatSync("/dojo.js", { bigint: true })
          ).toThrowError();
        }
      });
    ...
    describe(".statSync(path)", () => {
      ...
      it("Returns file stats using BigInt", () => {
        if (hasBigInt) {
          ...
          expect(() =>
            vol.statSync("/dojo.js", { bigint: true })
          ).toThrowError();
        }
      });
    ...
    describe(".mkdtempSync(prefix[, options])", () => {
      ...
      it("throws when prefix contains null bytes", () => {
        const vol = new Volume();
        expect(() => vol.mkdtempSync("/tmp-\u0000")).toThrow(
          /path.+string.+null bytes/i
        );
      });
    });
    describe(".mkdtemp(prefix[, options], callback)", () => {
      ...
      it("throws when prefix contains null bytes", () => {
        const vol = new Volume();
        expect(() => vol.mkdtemp("/tmp-\u0000", () => {})).toThrow(
          /path.+string.+null bytes/i
        );
      });
    });
