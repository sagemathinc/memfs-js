  ...
  return {
    ...
    appendFile(
      ...
      return promisify(vol, "appendFile")(
        path instanceof FileHandle ? path.fd : (path as PathLike),
        data,
        options
      );
    },
    ...
    open(path: PathLike, flags: TFlags, mode?: TMode): Promise<FileHandle> {
      return promisify(vol, "open", (fd) => new FileHandle(vol, fd))(
        path,
        flags,
        mode
      );
    ...
    readFile(
      ...
      return promisify(vol, "readFile")(
        id instanceof FileHandle ? id.fd : (id as PathLike),
        options
      );
    },
